...
export default class SQS {
  constructor(options) {
    if (!options || typeof options !== 'object') {
      throw new TypeError(
        `options must be an object. Provided value: ${options}`
      );
    }
    if (!options.awsRegion || typeof options.awsRegion !== 'string') {
      throw new TypeError(
        `options.awsRegion must be a string. Provided value: ${options.awsRegion}`
      );
    }
    if (!options.queueUrl || typeof options.queueUrl !== 'string') {
      throw new TypeError(
        `options.queueUrl must be a string. Provided value: ${options.queueUrl}`
      );
    }
    if (
      options.messageBatchSize &&
      typeof options.messageBatchSize !== 'number'
    ) {
      throw new TypeError(
        `options.messageBatchSize must be a number. Provided value: ${options.messageBatchSize}`
      );
    }
  ...
  async sendMessageBatch(messages) {
    if (!messages || !Array.isArray(messages)) {
      throw new TypeError(
        `Messages must be an Array. Value Provided: ${messages}`
      );
    }
